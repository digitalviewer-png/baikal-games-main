...
namespace Lean.Touch
{
    ...
    public class LeanTouch : MonoBehaviour
    {
        ...
        private static readonly int highestMouseButton = 7;
        ...
        private static readonly List<LeanFinger> filteredFingers = new List<LeanFinger>(10);
        ...
        public static float CurrentTapThreshold =>
            Instances.Count > 0 ? Instances[0].TapThreshold : DefaultTapThreshold;

        public static float CurrentSwipeThreshold =>
            Instances.Count > 0 ? Instances[0].SwipeThreshold : DefaultSwipeThreshold;

        public static int CurrentReferenceDpi => Instances.Count > 0 ? Instances[0].ReferenceDpi : DefaultReferenceDpi;

        public static LayerMask CurrentGuiLayers =>
            Instances.Count > 0 ? Instances[0].GuiLayers : (LayerMask) Physics.DefaultRaycastLayers;
        ...
        public static List<LeanFinger> GetFingers(bool ignoreGuiFingers, int requiredFingerCount = 0,
            LeanSelectable requiredSelectable = null)
        {
            ...
            for (var i = 0; i < Fingers.Count; i++)
            {
                ...
                if (ignoreGuiFingers)
                {
                    if (finger.StartedOverGui == false) filteredFingers.Add(finger);
                }
                else
                {
        ...
        private void BeginFingers()
        {
            ...
            for (var i = InactiveFingers.Count - 1; i >= 0; i--) InactiveFingers[i].Age += Time.unscaledDeltaTime;
            ...
            if (Input.touchCount > 0)
            {
                for (var i = 0; i < Input.touchCount; i++)
                {
                    ...
                    if (touch.phase == TouchPhase.Began || touch.phase == TouchPhase.Stationary ||
                        touch.phase == TouchPhase.Moved) AddFinger(touch.fingerId, touch.position);
                }
            }
        ...
        private void UpdateEvents()
        {
            ...
            if (fingerCount > 0)
            {
                for (var i = 0; i < fingerCount; i++)
                {
                    ...
                    if (finger.Set && OnFingerSet != null) OnFingerSet(finger);
        ...
        private void AddFinger(int index, Vector2 screenPosition)
        {
            ...
            if (finger == null)
            {
                ...
                if (inactiveIndex >= 0)
                {
                    ...
                    if (finger.Age > TapThreshold) finger.TapCount = 0;
