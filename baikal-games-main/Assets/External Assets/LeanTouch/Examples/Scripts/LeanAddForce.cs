...
namespace Lean.Touch
{
    ...
    {
        [Tooltip("The strength of the force")] public float ForceMultiplier = 1.0f;
        ...
        public void ApplyForce(LeanFinger finger)
        {
            ...
            for (var i = 0; i < LeanSelectable.Instances.Count; i++)
            {
                ...
                if (selectable.IsSelected || selectable.SelectingFinger == finger)
                {
        ...
        private void TryApplyForce(Component component, Vector3 force)
        {
            ...
            if (rigidbody != null) rigidbody.AddForce(force * ForceMultiplier);

            // Apply 2D force?
            var rigidbody2D = component.GetComponentInParent<Rigidbody2D>();

            if (rigidbody2D != null) rigidbody2D.AddForce(force * ForceMultiplier);
        }
    }
}