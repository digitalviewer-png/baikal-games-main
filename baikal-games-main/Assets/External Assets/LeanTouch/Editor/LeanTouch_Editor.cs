...
namespace Lean.Touch
{
    ...
    {
        private static readonly List<LeanFinger> allFingers = new List<LeanFinger>();

        private static GUIStyle fadingLabel;

        public static GUIStyle GetFadingLabel(bool active, float progress)
        {
            if (fadingLabel == null) fadingLabel = new GUIStyle(EditorStyles.label);

            var a = EditorStyles.label.normal.textColor;
            var b = a;
            b.a = active ? 0.5f : 0.0f;
            ...
            for (var i = 0; i < allFingers.Count; i++)
            {
                ...
                if (style.normal.textColor.a > 0.0f)
                {
                    ...
                    EditorGUILayout.LabelField(
                        "#" + finger.Index + " x " + finger.TapCount + " (" + Mathf.FloorToInt(screenPosition.x) +
                        ", " + Mathf.FloorToInt(screenPosition.y) + ") - " + finger.Age.ToString("0.0"), style);
                }
            }
        }
        ...
        {
            if (rightWidth == 0.0f) rightWidth = rect.width - EditorGUIUtility.labelWidth;

            var left = rect;
            left.xMax -= rightWidth;
            var right = rect;
